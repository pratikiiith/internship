...
//                BufferedReader reader = new BufferedReader(new InputStreamReader(getAssets().open("myfile.txt")));
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    // read one line // add to list
                    for(int i=0;i<c.length;i++) myList.add(Integer.parseInt(c[i]));
                    System.out.println(myList);
//                     create 3d points
                    int listsize = myList.size();
                    int temp = listsize/2;
                    Vector3f[] pp = new Vector3f[temp];
                    Vector2f[] vv = new Vector2f[temp];
                    int j=0;
                    for(int i=0;i<listsize;i=i+2){
                        vv[j] = new Vector2f(myList.get(i),myList.get(i+1));
                        j++;
                    }
                    ...
                    for (int i = 0; i < vv.length; i++) {
                        pp[i] = LineUtils
                                .GetWorldCoords(vv[i], mScreenWidth, mScreenHeight, projmtx, viewmtx);
                    }
//                    // write to file
                    File root = android.os.Environment.getExternalStorageDirectory();
                    System.out.println(root.getAbsolutePath());

                    String writefile = String.valueOf(writecount) + ".txt";
                    File dir = new File (root.getAbsolutePath() + "/Test");
                    dir.mkdir();
                    File file = new File(dir, writefile);
                    try {
                        FileOutputStream f = new FileOutputStream(file,true);
                        PrintWriter pw = new PrintWriter(f);
                        String s4 = "";
                        for(int k=0;k<pp.length;k++){
                            String s1 = String.valueOf(pp[k].x);
                            String s2 = String.valueOf(pp[k].y);
                            String s3 = String.valueOf(pp[k].z);
                            s4 = s4 + s1 + " " + s2 + " " + s3 + " ";
                        }
                        pw.println(s4);
                        pw.flush();
